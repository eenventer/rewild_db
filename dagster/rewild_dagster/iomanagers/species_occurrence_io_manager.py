...
class Species_Occurrence_IOManager(IOManager):
    def __init__(self):
        pass
    ...
    def handle_output(self, context: OutputContext, df: pd.DataFrame) -> None:
        ...
        dtype = {'location_id':String(), 'guid':String(), 'scientific_name':String(), 'common_name':String(), 'family':String(), 'kingdom':String(), 'count':Integer(), '_run_id':String(), '_dwh_processed_change_dtm':DateTime()}
...
@io_manager()
def species_occurence_io_manager(context):
    return Species_Occurrence_IOManager()

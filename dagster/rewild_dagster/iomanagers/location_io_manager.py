...
class Location_IOManager(IOManager):
    def __init__(self):
        pass
    ...
    def handle_output(self, context: OutputContext, df: pd.DataFrame) -> None:
        ...
        dtype = {'postcode':Integer(),'suburb':String(), 'state':String(), 'lat':DECIMAL(10,3), 'long':DECIMAL(10,3),'_run_id':String(), '_dwh_processed_change_dtm':DateTime()}
...
@io_manager()
def location_io_manager(context):
    return Location_IOManager()
